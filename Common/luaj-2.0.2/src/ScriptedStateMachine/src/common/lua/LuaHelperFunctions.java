...
final public class LuaHelperFunctions {
	...
	/**
	 * C++ version of this class delete pLua if exception occured
	 * But it is completely for nothing in Java :)
	 */
	...
	public static class LuabindObject {
		 ...
		 * @return value for this type, one of TNIL, TBOOLEAN, TNUMBER, TSTRING, TTABLE, TFUNCTION, TUSERDATA, TTHREAD
		 */
		public int type() {
			return val.type();
		}
		 ...
		 *
		 * @return name from type name list LuaValue.TYPE_NAMES
		 ...
		 *
		 * @throws LuaError - if this is not a table, or there is no INDEX metatag 
		...
		/**
		 * Call method with 0 arguments, including metatag processing, 
		 * and return only the first return value.
		 *
		 * <p>
		 * If <code>name</code> is a LuaFunction, call it, and return only
		 * its first return value, dropping any others. Otherwise, 
		 * look for the CALL metatag and call that.
		 * </p><p>
		 * If the return value is a Varargs, only the 1st value will
		 * be returned. To get multiple values,
		 * use LuaValue.invoke() instead.
		 * </p><p>
		 * To call this as a method call, use LuaValue.method(LuaValue)
		 * instead.
		 * </p>
		 * @return First return value (this()), or NIL if there were none. 
		 * 
		 * @throws LuaError if not a function and CALL is not defined,
		 * or the invoked function throws a LuaError or the invoked 
		 * closure throw a lua error
		 */
		...
		/**
		 * Call named method on this with 0 arguments,
		 * including metatag processing, and return only the first return value.
		 * <p>
		 * Look up this[name] and if it is a LuaFunction, call it
		 * inserting this as an additional first argument. and return
		 * only its first return value, dropping any others. Otherwise,
		 * look for the CALL metatag and call that.
		 * </p><p>
		 * If the return value is a Varargs, only the 1st value will
		 * be returned. To get multiple values, use LuaValue.invoke()
		 * instead.
		 * </p><p>
		 * To call this as a plain call, use LuaValue.call() instead.
		 * </p>
